...
class AttentionLayer(Layer):
    ...
    def build(self, input_shape):
        w_dim = input_shape[-1]
        self.W_s = self.add_weight(shape=(w_dim, w_dim),initializer=self.init,name='{}_Ws'.format(self.name), trainable=True)
        self.B_s = self.add_weight((w_dim,), initializer='zero', name='{}_bs'.format(self.name))
        self.Attention_vec = self.add_weight((w_dim,), initializer='normal', name='{}_att_vec'.format(self.name))
